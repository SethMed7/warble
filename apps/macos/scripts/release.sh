#!/bin/sh
# Builds a DISTRIBUTABLE, notarized warble.dmg — the thing people download and double-click.
#
...
PROFILE="${WARBLE_NOTARY_PROFILE:-voz-notary}"
...
WARBLE_SIGN_ID="$DEVID" sh scripts/bundle.sh
...
#    helpers get NO app entitlements (they don't take them); only the app gets warble.entitlements.
...
codesign --force --options runtime --timestamp --entitlements warble.entitlements -s "$DEVID" "$APP"
...
# 4. Build a BRANDED, drag-to-install DMG with dmgbuild — warble.app + an Applications shortcut, a dark
...
DMG="dist/warble-$VER.dmg"; rm -f "$DMG"
...
# Detach any stale "warble" volumes first — otherwise dmgbuild's temp volume mounts as "warble 1" and bakes
# a broken background-image alias into the .DS_Store, so the background silently won't render on open.
for v in /Volumes/warble*; do [ -e "$v" ] && hdiutil detach -force "$v" >/dev/null 2>&1 || true; done
WARBLE_APP="$PWD/$APP" WARBLE_BG="$PWD/media/dmg-bg.png" \
  "$DMGVENV/bin/dmgbuild" -s scripts/dmgbuild-settings.py "warble" "$DMG"
