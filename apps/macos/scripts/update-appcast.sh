#!/bin/sh
# Add a release to the Sparkle appcast so voz's in-app updater can offer it. Run AFTER release.sh has
...
#   e.g. sh scripts/update-appcast.sh 0.1.7 dist/voz-0.1.7.dmg
#
...
URL="https://github.com/SethMed7/voz/releases/download/v$VER/$(basename "$DMG")"
...
item = f'''    <item>
      ...
      <link>https://github.com/SethMed7/voz/releases/tag/v{ver}</link>
      <pubDate>{pub}</pubDate>
...
print(f"✓ appcast.xml updated for voz {ver}")
print(f"  enclosure: {url}")
PY
echo "Next: from the repo root,  git add appcast.xml && git commit -m \"appcast: voz $VER\" && git push"
