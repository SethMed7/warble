...
/// installed by scripts/setup-cleaner.sh into ~/.warble/llm. Same idea as Ollama —
...
final class LLMCleaner: Cleaner {
    ...
    static func binaryPath() -> String? {
        ...
        let candidates = [
            ProcessInfo.processInfo.environment["WARBLE_LLAMA_BIN"],
            "\(home)/.warble/llm/bin/llama-cli",
    ...
    /// A single .gguf model. WARBLE_LLM_MODEL overrides; otherwise the file the
    /// setup script drops at ~/.warble/llm/model.gguf.
    static func modelPath() -> String? {
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        let candidates = [
            ProcessInfo.processInfo.environment["WARBLE_LLM_MODEL"],
            ProcessInfo.processInfo.environment["VOZ_LLM_MODEL"], // rename-era fallback (voz ≤ 0.1.8)
            "\(home)/.warble/llm/model.gguf",
        ].compactMap { $0 }
