...
| `--render-home <out.png>` | Home (ROADMAP 0.6 dashboard retention pass) — no state argument; seed the scenario via `WARBLE_HOME` (empty, or a populated `history.json` + `learned.json`) |
| `--render-share-card <out.png>` | "Save a stats card" — the SAME renderer the live button calls, over `WARBLE_HOME`'s real stats; errors (exit 2) with nothing to share yet |

For human design review, one command renders all of them — every tour card and variant, every
Setup state, every pill state, both History-detail scenarios, and the dashboard retention pass's
Home (empty + populated) and share card (33 PNGs, `@2x`):
...
- **Context awareness, the live AX read**: turn on **Dashboard ▸ Data & Privacy ▸ Context
  ...
  spot-check that a chat dictation reads informal and an editor one keeps identifiers intact.
  (The rules and the prompt golden are the scripted twins — BasicCleanerTests/`clean.test.ts` +
  `--clean-in-context` + the `context-apply` check; this pass is the end-to-end feel.)
- **The dashboard retention pass, by eye**: open **Dashboard ▸ Home** after a few real dictations —
  the WPM/typist line and human-units line read naturally, the streak heatmap's tint ramp is
  legible against `ink` (DESIGN.md's One-Accent Rule: tints of electric only), and fixing an
  in-place correction enough times to promote it shows a **"learned: `<word>`"** row in the feed at
  the moment it happens. Click **"Save a stats card"**, pick a location in the real NSSavePanel, and
  open the PNG — the mark, the three headline stats, and the voice gradient should look like a
  real marketing asset, not an in-app screenshot (the one declared identity-surface exception, not
  a drift). (The math is unit-tested — RetentionTests/BasicCleanerTests — and both Home states plus
  the card itself render real PNGs via `--render-home`/`--render-share-card`, the `retention` check;
  this pass is the calibration and the live gesture.)
