...
The mark is **the trill** — five thick sound-wave bars whose envelope rises, crests, dips, and
crests again: the warble itself, sung in two syllables (*war-ble*). Deliberately few bars and
heavy strokes, so it stays unmistakable at 18 pt in the menu bar — visibility beats detail. It
runs on a **deep-royal → cyan gradient** (the wave enters in deep royal; the song crests in cyan).
Canonical art: `brand/warble-mark.svg` (color) and
`apps/macos/Sources/Shared/Resources/warble_glyph.svg` (monochrome) — both **generated by
`brand/source/gen_wave_mark.py`** (tune the bar heights there; never hand-edit the bars).
...
- **Both in-app panels are dark** (`ink #161520`): the read-along panel and the dictation pill are
  ...
  The **menu-bar icon is the trill mark** (`apps/macos/Sources/Shared/WarbleMark.swift`, loading
  `warble_glyph.svg`), drawn full-bleed as a template so it tints to the light/dark menu bar — five
  thick bars stay legible at 18 pt, which is exactly right for a voice tool.
